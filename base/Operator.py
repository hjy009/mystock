...
def buy(stock_code,opdate,buy_money):
    ...
    if deal_buy.cur_money_rest+1 >= buy_money:
        ...
        #dy = daily.daily(ts_code=done_set2[i][0])
        #dy.load_info()
        #done_set_buy = dy.data.loc[(dy.data['stock_code'] == done_set2[i][0]) & (dy.data['state_dt'] == state_dt)]
        done_set_buy = ccdy.i.df_stdate(ts_code=stock_code,from_date=opdate,to_date=opdate)
        if len(done_set_buy) == 0:
            return -1
        #buy_price = float(done_set_buy[0][3])
        buy_price = ccdy.i.valuebykey(ts_code=stock_code,st_date=opdate,key='close')
        if buy_price >= 195:
            return 0
        ...
        if vol == 0:
            return 0
        new_capital = deal_buy.cur_capital - vol * buy_price * shouxufei
        new_money_lock = deal_buy.cur_money_lock + vol * buy_price
        new_money_rest = deal_buy.cur_money_rest - vol * buy_price * (1+shouxufei)
        ...
        dic = {}
        dic['capital'] = new_capital
        dic['money_lock'] = new_money_lock
        dic['money_rest'] = new_money_rest
        dic['deal_action'] = 'buy'
        dic['stock_code'] = stock_code
        dic['stock_vol'] = vol
        dic['state_dt'] = opdate
        dic['deal_price'] = buy_price
        dic['seq'] = seq
        my_capital.i.append_dict(dic)
        if stock_code in deal_buy.stock_all:
            new_buy_price = (deal_buy.mprice[stock_code] * deal_buy.mvol[stock_code] + vol * buy_price) / (deal_buy.mvol[stock_code] + vol)
            new_vol = deal_buy.mvol[stock_code] + vol
            ...
            #db.commit()
            df = my_stock_pool.i.data.loc[(my_stock_pool.i.data['stock_code']==stock_code)]
            df['buy_price'].iloc[0]=new_buy_price
            df['hold_vol'].iloc[0]=new_vol
            df['hold_days'].iloc[0]=1
        else:
            ...
            #cursor.execute(sql_buy_update3)
            #db.commit()
            dic = dict(zip(my_stock_pool.i.default_columns(), [stock_code, buy_price, vol, int(1)]))
            my_stock_pool.i.append_dict(dic)
        #db.close()
        return 1
    #db.close()
    return 0
...
def sell(stock_code,opdate,predict):
    ...
    init_price = deal.mprice[stock_code]
    hold_vol = deal.mvol[stock_code]
    hold_days = deal.mdays[stock_code]
    ...
    done_set_sell_select = ccdy.i.df_stdate(ts_code=stock_code, from_date=opdate, to_date=opdate)
    if len(done_set_sell_select) == 0:
        return -1
    #sell_price = float(done_set_sell_select[0][3])
    sell_price = done_set_sell_select['close'].iloc[0]
    ...
    if sell_price > init_price*1.03 and hold_vol > 0:
        ...
        dic['money_lock'] = new_money_lock
        dic['money_rest'] = new_money_rest
        dic['deal_action'] = 'SELL'
        dic['stock_code'] = stock_code
        dic['stock_vol'] = hold_vol
        dic['profit'] = new_profit
        dic['profit_rate'] = new_profit_rate
        dic['bz'] = 'GOODSELL'
        dic['state_dt'] = opdate
        dic['deal_price'] = sell_price
        dic['seq'] = seq
        my_capital.i.append_dict(dic)
        #sql_sell_update = "delete from my_stock_pool where stock_code = '%s'" % (stock_code)
        #cursor.execute(sql_sell_update)
        #db.commit()
        my_stock_pool.i.dropkeyvalue('stock_code',stock_code)
        #db.close()
