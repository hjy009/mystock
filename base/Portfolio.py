...
def get_portfolio(stock_list,state_dt,para_window):
    ...
    #dts = cd.loc[(cd[0:].astype(str) >= model_test_date_start) & (cd[0:].astype(str) <= model_test_date_end) & (
    #    trade_cal.i.data['is_open'].astype(bool))]
    #date_temp = list(dts.astype(str))
    #model_test_date_seq = [(datetime.datetime.strptime(x, "%Y%m%d")).strftime('%Y%m%d') for x in date_temp]
    model_test_date_seq = trade_cal.date_range(model_test_date_start,model_test_date_end,1).astype(str)
    ...
    for i in range(len(model_test_date_seq)-4):
        ...
        for j in range(len(portfilio)):
            df = ccdy.i.df_stdate(portfilio[j],model_test_date_seq[i],model_test_date_seq[i + 4])
